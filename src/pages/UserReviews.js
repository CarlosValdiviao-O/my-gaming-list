...
const UserReviews = (props) => {
    ...
    const fetchReviews = async (isFirst) => {
        ...
        if (isFirst === true) {
            setReviews(null);
            reviewsRef = firestore.collection('reviews').where('userId', '==', id).orderBy('createdAt', 'desc').limit(21);
        }
        else {
            aux = reviews;
            reviewsRef = firestore.collection('reviews').orderBy('createdAt', 'desc').where('userId', '==', id).limit(21).startAfter(lastDoc);
            setReviews(null);
        }        
    ...
    return (
        <div id='reviews'>
            {(user && user.id === id) ? 
                <h1>Your Reviews</h1>
                :
                <h1>{name.replace(/_/g, ' ')}'s Reviews</h1>
            }
